...
const FeedbackAnimation: React.FC<FeedbackAnimationProps> = ({
  ...
  const iconElement = icon || DEFAULT_ICONS[type];
  ...
  return (
    ...
    >
      <div className={cn('flex-shrink-0 mr-3', ICON_STYLES[type])}>
        {iconElement}
      </div>
      ...
      {persistent && type !== 'loading' && (
        <button
          type="button"
          className="ml-auto -mx-1.5 -my-1.5 bg-transparent inline-flex items-center justify-center w-6 h-6 p-1 rounded-md focus:outline-none"
