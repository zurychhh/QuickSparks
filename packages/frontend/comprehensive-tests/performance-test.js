...
// URLs to test - when running locally
const URLS = [
  'http://localhost:3001/pdfspark/',
  'http://localhost:3001/pdfspark/convert'
];

// Create reports directory
const reportsDir = path.join(__dirname, '..', 'test-reports', 'lighthouse');
...
async function runLighthouseTests() {
  console.log('Starting Performance Testing with Lighthouse...');
  
  // Skip server check for now
  console.log('Skipping server check - assuming server is running on http://localhost:3001/pdfspark/');
  ...
  for (const url of URLS) {
    ...
    try {
      const { stdout, stderr } = await execAsync(
        `lighthouse ${url} --output=html,json --output-path=${reportPath} ` +
  ...
  if (summary.length > 0) {
    ...
    console.log('No successful Lighthouse tests completed.');
  }
}
